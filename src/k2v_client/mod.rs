...
impl K2vClient {
    ...
    pub async fn poll_item(
        ...
        let res = self
            .dispatch(
                req,
                Some(timeout + self.config.default_timeout.unwrap_or(DEFAULT_TIMEOUT)),
            )
            .await?;
    ...
    async fn dispatch(
        ...
        let res = tokio::select! {
            ...
            _ = tokio::time::sleep(timeout.unwrap_or(self.config.default_timeout.unwrap_or(DEFAULT_TIMEOUT))) => {
